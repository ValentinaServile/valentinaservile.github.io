...
The `-L` option is used to tell SSH that it should forward traffic from any port on our machine through the jump host and to a port on the destination server.

This is similar to what the `ProxyCommand` in the [Jumping Hosts](/blog/jumping-ssh-hosts/) post was doing, except that instead of us sending data through stdin, SSH will allocate a socket to listen on the port on the local side. Whenever a connection is made to this port, it will be forwarded to the secure channel.
...
Scenario: you want the remote server to access a specific application running on your local machine (which listens on a specific port)
...
Scenario: you want to reach an entire network (any application on any port) by using the jump host as a proxy for all your traffic

![](../../assets/blog/ssh-tunnelling-and-port-forwarding/image-11.png)

For example, we might have a lot of machines running webservers which are only accessible from a campus network. We don’t have access to that network, but somebody set up a bastion host for us which we can use as a proxy. However, setting up local port forwarding for each and every server is a tedious process, and we would rather tell our web browser to route all its traffic through the bastion.

Luckily the `-D` flag allows you to turn your SSH client into a rudimentary SOCKS proxy server. When a client connects to this port (your web browser, for example), the connection is forwarded to the jump host, which then forwards it to a dynamic port on any destination machine.
