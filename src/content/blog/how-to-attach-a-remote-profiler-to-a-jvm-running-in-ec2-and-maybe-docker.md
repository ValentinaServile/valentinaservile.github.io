...
For the purposes of this guide we will be using [JProfiler](https://www.ej-technologies.com/products/jprofiler/overview.html). Although any profiler which can work with an agent and a client can be used by following the same principles.
...
In order to bypass that limitation and still obtain a tunnel we can make use of the [ProxyCommand feature of SSH](/blog/jumping-ssh-hosts/) and send our traffic through an SSM command ran with the AWS CLI.
