...
description: 'In this post I am going to propose a setup to run any kind of application on a developer laptop in complete isolation.'
pubDate: '2021-05-15'
...
This could be a .jar or .war file, an entire directory, a binary file and so on.
...
We have successfully set up Docker around our application. Still, we haven’t changed its configuration at all and its dependencies don’t exist yet, so it will probably not work right away.  
...
What that looks like will depend heavily on the language and framework being used, but generally in most setups we have sets of key value pairs that are different for each environment, maybe stored in different files.  
...
Usually a major part of the configuration is the set of addresses at which the application can reach its dependencies in that particular environment, which we have conveniently left out of this section.  
...
Most applications make use of external persistence or messaging services.  
...
We will use the [mongo](https://hub.docker.com/_/mongo) image we found, which also conveniently lets us initialise any data needed in the database by adding JavaScript files to a `/docker-entrypoint-initdb.d/` folder.  
...
**Note on seeding data:** official images for well-known third-party services usually have some way to pre-populate data or a schema, like the MongoDB one in the example, and it can hopefully be used in the Dockerfile so that the step happens at image build time.  
However, some official images do not have such an API.  
We can bypass that restriction by adding a custom CMD in the Dockerfile: it should execute the service (in the background) plus our own script immediately after, then sleep to keep the container alive, like this:
...
Finally we can change our application’s configuration for running in the Docker environment so that it talks to our mongo container instead of trying to connect to the real Mongo Atlas cluster over the internet.  
We will leverage the networking features of docker-compose, which allow a container to resolve the name of any service declared within the same `docker-compose.yaml` file.  
This means our application container can resolve the name “mongo” to the correct container IP address without any further configuration of the Docker network:
...
However, we probably don’t want to rebuild the mock images every time, as they will change much less frequently than our application image and rebuilding them would slow down the script unnecessarily.  
...
if we have made any change to the mocks supporting the application, like the seed data for example.  
...
Azure and Google Cloud Platform also have their own ways of reproducing their services locally, so it is worth checking their documentation too, although they are outside the scope of this guide.  
...
The clients for other AWS services (and for all other languages) all allow us to change this configuration, so we can do the same with pretty much any other service we need.  
